...
int main()
{
    ...
    for (ii = 0; ii < 4; ii++) {
        //call the 4 argument find function with values from the 2 above arrays
        index = find(B, upcToSearch[ii], titlesToSearch[ii]); //****TODO 2:
        if (index >= 0) {
            //add the found object at the corresponding index to cCol2. (Review the Collection code for the correct operator) 
    ...
    for (ii = 0; ii < 6; ii++) {
        if (find(P, 6, "Dog")) { //****TODO 1:
            loadCollection(pCol2, P[index]); //****TODO 4:
        }
    }
    ...
    for (ii = 0; ii < 6; ii++) {
        if (find(a, 6, 90)) { //****TODO 1:
            loadCollection(iCol, a[index]); //****TODO 4:
        }
    }
